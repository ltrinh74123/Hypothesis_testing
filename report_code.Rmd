...
The survey issued to students had several response biases, including asking certain questions like their Weighted Average Score (WAM) where students may have felt uncomfortable disclosing their responses. The weight average score is an average score that represents the students' course results. While most people avoided answering the question, some provided implausible replies, such as having a WAM of zero. Furthermore, because the students would be required to do data analysis based on the results, 'demand characteristics' might have influenced their replies, where they may have had two objectives in mind:

1.  Provide perfect replies that are most likely in line with general population expectations.
2.  Give a ridiculous response

Furthermore, the survey featured 38 questions with a lot of multiple choices, which [@survey_effects] discovered increases participant boredom, which reduces reliability and completion rates since it encourages people to start choosing random responses.
...
Other modifications include altering the actual questions because they were relatively irrelevant to other factors, such as 'Pick a random number,' and focusing on a specific trait (such as their WAM) so that the questions arevolves around attributes that may affect them to obtain higher quality data.
